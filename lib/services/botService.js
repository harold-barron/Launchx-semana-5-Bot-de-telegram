...
const explorer = require("./ExplorerService")
class botService
{
    static createBot(token,explorer)
    {
        ...
        bot.on("message", (msg) => {
            const chatId = msg.chat.id;
            console.log(mas.text)
            const numberToApplyFb = parseInt(msg.text);
            if (msg.text === "node")
            {
                const explorersInNode = explorer.filterByMission(explorer,msg.text)
                console.log(explorersInNode)
                bot.sendMessage(chatId, explorersInNode[1]);
            }
            else if (msg.text === "java")
            {
                bot.sendMessage(chatId, "buscando alumnos en java");
            }
